...
def carregaDetalhes(request, provaId):
    ...
    for tipo in tipos[filtrarPor]:
        qntQuestoesTotais.append(fazBuscaQuestoes(prova,filtrarPor,tipo,'all').count())
    qntQuestoesTotais.append(f'{fazBuscaQuestoes(prova,filtrarPor,'all','all').count()}/{questoesTotais.count()}')
    ...
    for materia in materias:
        ...
        for tipo in tipos[filtrarPor]:
            ...
            qntQuestoesPorMateria[materia.nome].append(fazBuscaQuestoes(prova,filtrarPor,tipo,materia).count())
        totalPorStatus = fazBuscaQuestoes(prova, filtrarPor, 'all', materia=materia).count()

        
        porcentagem = f'{totalPorStatus / fazBuscaQuestoes(prova,status='all', tipo='all', materia=materia).count()  *100:.2f}%' if totalPorStatus != 0 else f'0%'
        qntQuestoesPorMateria[materia.nome].append(totalPorStatus)
        qntQuestoesPorMateria[materia.nome].append(porcentagem)
        
   
    printDict(qntQuestoesPorMateria)
