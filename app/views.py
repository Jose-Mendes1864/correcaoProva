...
def listarProvas(request):
    ...
    for prova in provas:
        dicionarioQuantQuest[prova.id] = Questoes.objects.filter(prova=prova).count()
        prova.status_value = StatusEnum[prova.status].value
    print(dicionarioQuantQuest)
...
def corrigirProva(request, id):
    ...
    for area in areasProva:
        ...
        for materia  in Materia.objects.filter(areaPertence_id=area.id):
           ...
           qntQuestoesPorMateria[materiasDaProva] = Questoes.objects.filter(materia_id=materia.id).count()
